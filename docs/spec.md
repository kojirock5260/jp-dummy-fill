...
欄種を 4 系統足した。判定・値の規則は `docs/field-rules.md` §2.1（ローマ字）、§2.6（希望日）、
§2.7（法人番号・インボイス）、§2.10（クレジットカード）、決定事項は §7 の 8〜11。
既存の欄種・データ・UI は変えていない。seed 0 の人物も 0.1.0 と同じ（乱数は末尾に足した）。

ストア用の zip は `npm run pack`（`scripts/pack.ts`）で作る。Edge 用はショートカットの既定を
`Alt+Shift` に載せ替える。Edge 本体が `Ctrl+Shift+Y`（コレクション）`U`（読み上げ）`K`（タブ複製）
を使っていて、拡張には渡らないため。
